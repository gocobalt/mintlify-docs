...
### Using Cobalt API
Use the [Upsert Linked Account API](https://docs.gocobalt.io/api-reference/customer/upsert-a-linked-account) call below to create or update a linked account.
```bash cURL
curl --request PUT \
...
The API will respond with a JSON Object containing data about the Linked Account created/updated if successful.
``` bash Response
{
...
You can also use the **.upsertLinkedAccount()** method of the [NodeJS SDK](https://docs.gocobalt.io/sdks/server-side-sdks/nodejs-sdk) to create or update Linked Account.
