...
1. **Request** : Make a POST request to the `/event/trigger/:slug` endpoint. `linked_account_id` is a mandatory field to be passed while making the request.
``` JavaScript Request
curl --request POST \
...
3. **Handle Errors** : If the `/event/trigger/:slug` endpoint returns a 400 error, your server-side code should handle it gracefully. The error is returned if you provided the wrong name of the event.
```JavaScript 400
{
    "status_code": 400,
