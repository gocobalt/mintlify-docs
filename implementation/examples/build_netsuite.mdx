...
<Step title="Mapping Contact fields">
  ...
  Click on `+ Map Fields` under Input Parameters, add key name as `contacts` and in value we will provide the response received through NetSuite node which will be restructured.

  In `Value`, select **Nodes** tab under Insert Variable and click on `+` of the NetSuite Node.
...
const res = params.contacts.map(ele=>{
        const output =  {
            id: ele.Id,
            title: ele.Name,
...
You have successfully created a NetSuite workflow to sync all the opportunities to your system.
</Check>
</Accordion>
