...
Within the workflow, add a rule node along with start and app nodes to define the workflow flow. Connect the nodes in sequence: **start node > rule node > app node (add action)**.
