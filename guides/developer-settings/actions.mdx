...
<img height="200" src="/images/api_proxy.png" alt="API Proxies in Cobalt"/>
Above image displays where you can find the API Proxy or Custom Actions that you have created and also the settings required to create a new API Proxy.
...
To create an API Proxy in Cobalt, the user needs to provide specific information detailing the desired behavior of the action. This includes the following steps:
    ...
    <img height="200" src="/images/create_proxy.png" alt="Create an API Proxy in Cobalt"/>

    ### 2. Configuring the API Proxy
    While configiring an API Proxy, there are 3 options that can be used to configure and make changes.
    ...
    And the `API Call` tab is used to define the API call to be executed upon triggering the action, Specify the endpoint, HTTP method, and any required request parameters.
        #### 2.1 Adding a Field
        - To add a new Field, click on the `Edit` button in **Fields** and click on **text** to provide additonal details.
        - Information about the Field like `Type` of the field, `Label`, `Placeholder`, `Description` and whether the Field is Mandatory or not can be configured.
            <img height="200" src="/images/field_proxy.png" alt="Create an API Proxy Field"/>   

        #### 2.2 API Call Configuration
        - Click on `Edit` button in `API Call`.
        ...
        <img height="200" src="/images/call_proxy.png" alt="Create an API Call for Custom Action"/> 

### Utilizing the API Proxy in Workflows
Once created, API Proxies can be used in workflows for various applications within Cobalt. This means that one Custom Action can be executed in several workflows. Users can define trigger conditions that initiate the execution of these actions, enabling automated responses to specific events and facilitating end-to-end process automation.
<img height="200" src="/images/proxy_workflow.png" alt="Executing Action in a Workflow"/> 