...
Cobalt makes use of the API credentials and Access tokens authorised by your end customers to authenticate the API calls to an application. 
...
For such scenarios Cobalt flags an application integration for `reauth-required` for the customer and they would need to re-authenticate for the workflows to function again.
...
<Note>Checkout [Cobalt webhooks](https://docs.gocobalt.io/guides/platform/webhook) for more details.</Note>