...
5. Go to your [`Apps catalog`](https://app.refold.ai/applications) in **Refold** > Search for `Google Admin` > `Settings` > `Use your credentials` > `Callback Url` > Copy it.
...
Navigate to  `Cloud Console dashboard` > Select the **Project** created for Refold > Follow the steps 8 and 9 mentioned [above](/applications/providers/google_admin#creating-an-app-in-google-admin) for adding scopes. 
