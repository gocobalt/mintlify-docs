...
2. HubSpot OAuth app credentials added to Refold. Refer [here](/applications/providers/hubspot#acquiring-hubspot-client-credentials).
3. Trigger created in Refold. Refer [here](/v3/platform/concepts/workflows/triggers/overview).
4. Linked Account available. Refer [here](/applications/providers/hubspot#acquiring-hubspot-client-credentials).

## Building a Workflow

#### 1. Create Workflow in app
* Go to your [`Apps catalog`](https://app.refold.ai/applications) in **Refold** > Search for `HubSpot` > `Workflows` > `Add Workflow` > Choose `Manually` > Give it a name and click on `Add Workflow`.
