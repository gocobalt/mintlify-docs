...
---
To setup your Shopify app in Refold for OAuth, you will need the following credentials from your [Shopify Partner Dashboard](https://partners.shopify.com/current/resources):
- Client ID
- Client Secret
- Scopes
...
5. In **Refold**, go to your [`Apps catalog`](https://app.refold.ai/applications) > Search for `Shopify` > `Settings` > `Use your credentials` > `Callback Url`. Copy the Callback URL as provided in your Shopify settings page.
...
Once you have tested the Shopify integration with Refold using a test shop, it is time to publish your application so that other production shops of different organizations can install/authorize your application.

Shopify mandates publishing and listing of your application in order to be installed by other users from different organizations. This requires a review from the Shopify team who make sure that the application is installed through their marketplace as well and not just through the installation URL generated through Refold’s SDKs.
...
```
<Note>
Replace the `Client ID` and `Client Secret` with your valid credentials. This will provide you with a response consisting of the `access token` and `shop name`. Migrate the acquired token to Refold.
</Note>

#### 4. Migrating access token to Refold

Once the access token is acquired, send it to Refold for further handling API authentications, token expiries and retries.
```JavaScript
curl --location --request PUT 'https://app.refold.ai/api/v2/public/migrate-token' \
--header 'x-api-key: {{Refold API KEY}}' \
...
Copy the `Single webhook URL` from the Refold Shopify app setting page and paste them as Shopify `Compliance webhooks` URLs in the Shopify app configuration page. Click `Save and release` at the top to save your changes.
