...
## Pre-requisites
1. Slack App is enabled in Refold.
2. Slack OAuth app credentials added to Refold. Refer [here](/applications/providers/slack#creating-an-app-in-slack).
3. Trigger created in Refold. Refer [here](/v3/platform/concepts/workflows/triggers/overview).
4. Linked Account available. Refer [here](/applications/providers/slack#creating-an-app-in-slack).

## Building a Workflow

#### 1. Create Workflow in app
* Go to your [`Apps catalog`](https://app.refold.ai/applications) in **Refold** > Search for `Slack` > `Workflows` > `Add Workflow` > Choose `Manually` > Give it a name and click on `Add Workflow`.
