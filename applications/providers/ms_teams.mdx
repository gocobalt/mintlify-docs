...
2. A bot registered in the Microsoft Developer Portal for Teams (See [Creating a Teams bot](/applications/providers/ms_teams#creating-a-teams-bot) for setup instructions).
...
4. Go to your [`Apps catalog`](https://app.refold.ai/applications) in **Refold** > Search for `Microsoft Teams` > `Settings` > `Use your credentials` > `Callback Url` > Copy it.
...
Select the OAuth App created for Refold and follow **Step 6** and **7** [above](/applications/providers/ms_teams#creating-an-app-in-microsoft-azure).
