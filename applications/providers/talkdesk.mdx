...
3. Go to your [`Apps catalog`](https://app.refold.ai/applications) in **Refold** > Search for `Talkdesk` > `Settings` > `Use your credentials` > `Callback Url` > Copy it.
...
6. Select the **mandatory scopes** in the **Selected scopes** section, as provided in your Refold Talkdesk settings page and click on the `Add Scopes` button. Click on `Create` to complete the app creation.
