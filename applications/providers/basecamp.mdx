...
4. Go to your [`Apps catalog`](https://app.refold.ai/applications) in **Refold** > Search for `Basecamp` > `Settings` > `Use your credentials` > `Callback Url` > Copy it. Paste the Callback URL under `Redirect URI` in Basecamp and click on the `Register this app` button.
