...
2. Required Settings for the App. You can find it in [Overview](/applications/providers/sheets_overview#required-settings).
...
5. Go to your [`Apps catalog`](https://app.refold.ai/applications) in **Refold** > Search for `Google Sheets` > `Settings` > `Use your credentials` > `Callback Url` > Copy it.
...
Navigate to  `Cloud Console dashboard` > Select the **Project** created for Refold > Follow the steps 8 and 9 mentioned [above](/applications/providers/google_sheets#creating-an-app-in-google-drive) for adding scopes. 
