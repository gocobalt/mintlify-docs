...
One of the use-cases is: You want to fire the [Workflow API](https://docs.gocobalt.io/build/workflow/workflow_api) of Cobalt to execute a workflow when some event occurs in Cobalt. You will simply add the headers and Body too if required.
...
Also, if a workflow with [Response node](https://docs.gocobalt.io/build/workflow/response) in it got executed successfully and you want to use it's response data during Webhook firing, you can also template that in Webhook.
