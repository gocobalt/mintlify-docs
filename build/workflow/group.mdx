...
Consider you want to access an `Id` that is unique within each array item that is being passed in the Loop Node. To do so, call the Loop Node as a Variable under `Nodes` and write it as `{{group:array_item.Id}}`.
