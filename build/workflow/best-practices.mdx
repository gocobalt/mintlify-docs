...
<Note>
  PII masking is an upcoming platform feature. Check the [changelog](/changelog/overview) for availability in your deployment version. Until it is available, use encrypted environment variables combined with `Hide Node Request` as the primary approach.
</Note>
...
The **HTTP node** is a standalone canvas node. Any credentials it uses must be explicitly provided — via an environment variable, a hardcoded value, or a value templated from a prior node. All of these surface in execution logs unless `Hide Node Request` is enabled or PII masking is configured.
...
<Note>
  PII masking is an upcoming platform feature. Check the [changelog](/changelog/overview) for availability in your deployment version.
</Note>
