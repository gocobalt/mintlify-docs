...
---
<Tip>
You can refer to the Refold's GitHub repo for the NodeJS SDK [here](https://github.com/gocobalt/cobalt-sdk).
</Tip>
#### Install
...
```
The Refold SDK for JavaScript bundles TypeScript definition files for use in TypeScript projects. We aim to keep these TypeScript definition files updated with each release for any public api.
...
The "Create Linked Account" fucntion allows you to create a new linked account for your customer within Refold's platform. Linked accounts represent your 
...
The **Upsert Linked Account** function allows you to create or update a linked account for your customer within Refold's platform. Linked accounts represent your end-users or customers, and they are essential for enabling connections with third-party applications. The API will either create a new linked account if it does not exist or update the existing linked account if it is already present.
...
The "Session Token" API is used to generate a secure session token that provides authorized access to Refold's APIs for a specific linked account. The session token serves as a temporary authentication credential for accessing the Refold JS SDK. 
