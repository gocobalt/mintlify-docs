...
A **workflow** is an automation that moves and transforms data between your application and your users' [linked accounts](/v3/platform/concepts/linked-account). You build it on a visual canvas from a [trigger](/v3/platform/concepts/workflows/triggers/overview) and a sequence of [nodes](/v3/platform/concepts/workflows/nodes/http). Use workflows to sync records, react to events in real time, and let non-technical teams configure integrations without writing code.
...
- **Nodes run in order**, each passing its output to the next through [data variables](/v3/platform/concepts/data-variables).
...
<Note>A workflow has exactly one trigger. To start the same logic in more than one way, build a workflow per trigger or call a shared workflow with the [Sub Flows node](/v3/platform/concepts/workflows/nodes/sub-flows).</Note>
...
See [Triggers](/v3/platform/concepts/workflows/triggers/overview) for how to choose and configure each one.
...
- **Utility nodes** — Refold's built-in steps for control flow and data handling, such as [Rule](/v3/platform/concepts/workflows/nodes/rule), [Loop](/v3/platform/concepts/workflows/nodes/loop), [Transform](/v3/platform/concepts/workflows/nodes/transform), [Custom Code](/v3/platform/concepts/workflows/nodes/custom-code), and [HTTP](/v3/platform/concepts/workflows/nodes/http). They don't call a third-party API themselves.
...
<Steps>
  <Step title="Set the trigger">
    Open the **Start node** and choose how the workflow starts. See [Triggers](/v3/platform/concepts/workflows/triggers/overview).
  </Step>
  ...
  <Step title="Configure each node">
    Set each node's fields, referencing data from earlier nodes with [data variables](/v3/platform/concepts/data-variables) and [templating](/v3/platform/concepts/workflows/templating).
  </Step>
  <Step title="Test the workflow">
    Use the **Run** tab to execute with sample data and verify the output before shipping. See [Testing](/v3/platform/concepts/workflows/testing).
  </Step>
  <Step title="Deploy">
...
<CardGroup cols={2}>
  <Card title="Triggers" icon="bolt" href="/v3/platform/concepts/workflows/triggers/overview">
    Choose how your workflow starts.
  </Card>
  <Card title="Nodes" icon="diagram-project" href="/v3/platform/concepts/workflows/nodes/http">
    Reference for every node you can add to a workflow.
  </Card>
  <Card title="Data variables & templating" icon="brackets-curly" href="/v3/platform/concepts/data-variables">
    Pass data between nodes and reference it in fields.
  </Card>
  <Card title="Quickstart" icon="play" href="/v3/platform/getting-started/quickstart">
