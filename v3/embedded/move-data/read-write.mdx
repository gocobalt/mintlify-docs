...
A single action call is the right tool when one operation does the job. Reach for a [workflow](/v3/developer/configure/workflows/overview) when the job spans more than one step.
...
| The call is synchronous and you want the result back | A [trigger](/v3/developer/configure/workflows/triggers/overview) (event, webhook, or schedule) should start the run |
| No mapping or validation is needed | You map and validate data between systems (see [Data mapping](/v3/embedded/move-data/data-mapping)) |

<Note>
A workflow is built once on the Platform and runs across all three Refold products. This page does not re-document workflow building. See [Workflows overview](/v3/developer/configure/workflows/overview).
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Data mapping" icon="arrow-right-arrow-left" href="/v3/embedded/move-data/data-mapping">
  ...
  <Card title="Workflows overview" icon="diagram-project" href="/v3/developer/configure/workflows/overview">
