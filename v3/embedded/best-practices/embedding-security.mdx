...
| **Session token** | One customer's frontend session | Generated server-side, passed to the browser | A single [linked account](/v3/developer/configure/linked-account), short-lived |
...
<Steps>
  ...
  <Step title="Pass it to the frontend">
    Hand the token to the browser and use it to render the connect experience, whether that's the [React SDK flow](/v3/embedded/frontend/react-sdk-flow), a [hosted flow](/v3/embedded/frontend/hosted-flow), or a [build-your-own](/v3/embedded/frontend/customization/build-your-own) UI.
  </Step>
...
<Note>
A session token is scoped to one linked account and expires (the [hosted flow](/v3/embedded/frontend/hosted-flow) generates a new connect URL per session). Generate it per customer, per session, on the server. Never share one token across customers.
</Note>

## Scope every call to one linked account

Multi-tenant isolation depends on scoping. Each customer is a [linked account](/v3/developer/configure/linked-account), and every account-scoped call carries that customer's `linked_account_id`.
...
- **Let Refold hold the tokens.** Customer OAuth and API credentials are stored encrypted against their linked account and used automatically. Your code never handles raw third-party tokens. See [token refresh](/v3/embedded/frontend/management/token-refresh).
...
- [Auth overview](/v3/developer/auth/overview) : how Refold authenticates apps and customers
- [Linked accounts](/v3/developer/configure/linked-account) : the per-customer scope every call carries
