...
For a no-code option with no React, use the [Hosted flow](/v3/embedded/frontend/hosted-flow). For a fully bespoke UI, [build your own](/v3/embedded/frontend/customization/build-your-own).
...
The `<Config>` component handles the whole experience: connection status, initiating OAuth, rendering your [config fields](/v3/embedded/config-fields), and saving the customer's choices.
...
<Tip>
See the [React SDK](/v3/sdks/client-side/react-sdk) reference for every `<Provider>` and `<Config>` prop, a complete example, and troubleshooting.
</Tip>
...
<CardGroup cols={2}>
  <Card title="React SDK reference" icon="react" href="/v3/sdks/client-side/react-sdk">
