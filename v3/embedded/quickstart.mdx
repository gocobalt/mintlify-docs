...
- **An app to integrate.** This guide uses HubSpot (`hubspot`) as the example. Any [connector](/v3/connectors/supported-apps-actions) works; swap the slug and action names to match yours.
...
<Note>
Testing? Use a sandbox account for the app so you can run the full loop without touching production data.
</Note>

## Step 3: Create a linked account

A [linked account](/v3/concepts/linked-account) represents one of your customers. You choose its `linked_account_id` and reuse that id in every account-scoped call. Create one per customer, typically when they sign up.
...
Your customer authorizes the app from your product. The fastest way to see it work is the hosted [Connect portal](/v3/embedded/connect-flow): generate a URL for the linked account and open it.
...
<Tip>
For multi-step logic (read, transform, then write, or fan out to several apps), run a [workflow](/v3/workflows/overview) instead of a single action.
</Tip>
...
<Steps>
  ...
  <Step title="Embed the connect flow in your product">
    Replace the hosted portal with the [React SDK](/v3/authentication/connection-flows/react-sdk) or a [build-your-own](/v3/authentication/connection-flows/build-your-own) flow so customers connect without leaving your app.
  </Step>
  <Step title="Handle connection health">
    Subscribe to [webhooks](/v3/connectors/my-app/webhooks) and surface [connection status](/v3/authentication/managing-connections/connection-status) so customers can reconnect when a token expires.
  </Step>
  <Step title="Run the go-live checklist">
    Walk the [go-live checklist](/v3/embedded/ship-operate/go-live) before your first customer.
  </Step>
</Steps>
