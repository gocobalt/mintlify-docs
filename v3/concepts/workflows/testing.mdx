...
<Steps>
  ...
  <Step title="Verify in production">
    Trigger one real event and check the run in your [workflow run history](/v3/concepts/workflows/overview). {/* NEEDS INPUT: confirm the run-history page path */}
  </Step>
</Steps>
...
- **Use Log nodes as breakpoints.** Drop [Log nodes](/v3/concepts/workflows/nodes/log-node) between steps to checkpoint intermediate values during a full-workflow run.
- **Wrap risky calls in Try/Catch.** Test the catch branch by feeding a payload guaranteed to fail (bad ID, missing field). See [Try/Catch nodes](/v3/concepts/workflows/nodes/try-catch-node).
...
<CardGroup cols={2}>
  <Card title="Workflow overview" href="/v3/concepts/workflows/overview">
    The fundamentals of workflows, triggers, and nodes.
  </Card>
  <Card title="Templating and variables" href="/v3/concepts/workflows/overview">
  ...
  <Card title="Log node" href="/v3/concepts/workflows/nodes/log-node">
