...
Each [Linked Account](/v3/concepts/linked-account) keeps its own Data Refs, even when names match across accounts, so one customer's mappings never leak into another's.
...
You read from a Data Ref inside a [workflow](/v3/concepts/workflows/overview) by adding the Data Ref node and choosing one of the lookup actions. Reads work in both directions, so you can start from either side of the mapping.
...
<CardGroup cols={2}>
  ...
  <Card title="Linked Accounts" icon="user-plus" href="/v3/concepts/linked-account">
  ...
  <Card title="Variables and references" icon="brackets-curly" href="/v3/concepts/data-variables">
