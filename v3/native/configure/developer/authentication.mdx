...
When Refold calls your app's API — to run an [API proxy](/v3/native/configure/developer/api-proxies) or write data back into your system — it has to authenticate as the customer whose data it's touching. **My App authentication** is where you set that up once: you store each customer's credentials on their [linked account](/v3/platform/concepts/linked-account), optionally exchange them for a short-lived token, and Refold injects the result into every API proxy call automatically.
