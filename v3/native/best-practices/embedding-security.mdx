...
| **Session token** | One customer's frontend session | Generated server-side, passed to the browser | A single [linked account](/v3/concepts/linked-account), short-lived |
...
Multi-tenant isolation depends on scoping. Each customer is a [linked account](/v3/concepts/linked-account), and every account-scoped call carries that customer's `linked_account_id`.
...
- [Linked accounts](/v3/concepts/linked-account) : the per-customer scope every call carries
