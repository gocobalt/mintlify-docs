...
<Steps>
  ...
  <Step title="Pass it to the frontend">
    Hand the token to the browser and use it to render the connect experience, whether that's the [React SDK flow](/v3/native/frontend/react-sdk-flow), a [hosted flow](/v3/authentication/connection-flows/hosted-flow), or a [build-your-own](/v3/authentication/connection-flows/build-your-own) UI.
  </Step>
...
<Note>
A session token is scoped to one linked account and expires (the [hosted flow](/v3/authentication/connection-flows/hosted-flow) generates a new connect URL per session). Generate it per customer, per session, on the server. Never share one token across customers.
</Note>
...
- **Let Refold hold the tokens.** Customer OAuth and API credentials are stored encrypted against their linked account and used automatically. Your code never handles raw third-party tokens. See [token refresh](/v3/authentication/managing-connections/token-refresh).
...
- [Auth overview](/v3/authentication/overview) : how Refold authenticates apps and customers
