---
title: "How It Works"
description: "The architecture behind Embedded Marketplace: how your app, Refold, and your customers' connected apps fit together, and what runs where."
---

An Embedded Marketplace integration connects three systems: your product, Refold, and the third-party apps your customers use. Your product owns the user experience and the data, Refold owns authentication and execution, and the third-party app owns the customer's data on the other side. Understanding which system does what is the fastest way to design an integration that is secure and easy to operate.
...
- **Your backend** holds your API key, creates one [linked account](/v3/concepts/linked-account) per customer, mints short-lived [session tokens](/v3/native/frontend/overview#before-you-start) for the frontend, triggers workflows and actions, and receives [webhooks](/v3/connectors/my-app/webhooks).
...
The four objects below combine to turn one connector into a working, per-customer integration. Each is defined once on the platform; this is how they relate in an Embedded Marketplace build.
...
<Note>
Workflows are built and configured on the platform, not in Embedded Marketplace. Embedded Marketplace is where you embed the connect flow and [expose workflows](/v3/native/frontend/expose-workflow) to your customers. See [Workflows](/v3/native/configure/integration/workflow) for the Embedded Marketplace view.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Linked accounts" icon="user" href="/v3/concepts/linked-account">
