...
- **Your backend** holds your API key, creates one [linked account](/v3/concepts/linked-account) per customer, mints short-lived [session tokens](/v3/native/frontend/overview#before-you-start) for the frontend, triggers workflows and actions, and receives [webhooks](/v3/native/configure/developer/webhooks).
...
- **Refold** runs the connection and [auth flows](/v3/platform/authentication/overview), stores and refreshes credentials, and executes every [workflow](/v3/concepts/workflows/overview) and action using the stored credential for the right linked account.
...
- **[Workflow](/v3/concepts/workflows/overview)** is the automation that moves data. It reads config field values and connected-app data at run time, so the same workflow behaves correctly for every customer.
