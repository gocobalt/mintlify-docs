...
<Note>
Test-run variables require the workflow testing prerequisites to be satisfied. See [Workflow Testing](/v3/workflows/testing) for the full setup.
</Note>
...
- Property names that contain dots or special characters are not supported through dotted access. If your JSON has such keys, restructure the upstream response (for example, via a [Transform node](/v3/workflows/nodes/transform)) before referencing it.
...
<CardGroup cols={2}>
  ...
  <Card title="Transform node" icon="wand-magic-sparkles" href="/v3/workflows/nodes/transform">
