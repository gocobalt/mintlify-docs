...
<Note>A workflow has exactly one trigger. To start the same logic in more than one way, build a workflow per trigger or call a shared workflow with the [Sub Flows node](/v3/workflows/nodes/sub-flows).</Note>
...
See [Triggers](/v3/workflows/triggers/overview) for how to choose and configure each one.
...
- **Utility nodes** — Refold's built-in steps for control flow and data handling, such as [Rule](/v3/workflows/nodes/rule), [Loop](/v3/workflows/nodes/loop), [Transform](/v3/workflows/nodes/transform), [Custom Code](/v3/workflows/nodes/custom-code), and [HTTP](/v3/workflows/nodes/http). They don't call a third-party API themselves.
- **App actions** — prebuilt actions for a connected app, like creating a Salesforce contact or fetching a NetSuite invoice. The available actions come from the app's [connector](/v3/connectors/supported-apps-actions).
...
<Steps>
  <Step title="Set the trigger">
    Open the **Start node** and choose how the workflow starts. See [Triggers](/v3/workflows/triggers/overview).
  </Step>
  ...
  <Step title="Configure each node">
    Set each node's fields, referencing data from earlier nodes with [data variables](/v3/concepts/data-variables) and [templating](/v3/workflows/templating).
  </Step>
  <Step title="Test the workflow">
    Use the **Run** tab to execute with sample data and verify the output before shipping. See [Testing](/v3/workflows/testing).
  </Step>
  <Step title="Deploy">
...
<CardGroup cols={2}>
  <Card title="Triggers" icon="bolt" href="/v3/workflows/triggers/overview">
    Choose how your workflow starts.
  </Card>
  <Card title="Nodes" icon="diagram-project" href="/v3/workflows/nodes/http">
