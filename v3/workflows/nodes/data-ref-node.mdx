...
<Tip>Use a [Rule Node](/v3/workflows/nodes/rule) after "Get Ref" actions to branch your workflow based on whether a mapping was found, enabling create-or-update logic.</Tip>

## What's Next

- [Rule Node](/v3/workflows/nodes/rule) to add conditional logic based on ref lookup results
- [Loop Node](/v3/workflows/nodes/loop) to process bulk refs from array data
- [Tables Node](/v3/workflows/nodes/tables) for more complex data storage needs
- [HTTP Node](/v3/workflows/nodes/http) to sync data with external systems after ref lookup