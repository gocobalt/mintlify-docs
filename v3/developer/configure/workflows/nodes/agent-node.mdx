...
<ParamField path="Input Files (URLs)" type="text area">
  A JSON array of file URLs to provide as additional context to the agent. Use this to pass documents, images, or other files for the model to analyze alongside the prompt. Supports dynamic variables from previous nodes using `{{variable}}` syntax.
</ParamField>
...
<Steps>
  ...
  <Step title="Add input files (optional)">
    Provide a JSON array of file URLs in the **Input Files (URLs)** field to give the agent documents or images as additional context.
  </Step>
