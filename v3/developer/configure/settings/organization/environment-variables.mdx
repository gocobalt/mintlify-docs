...
description: "Store workspace-scoped configuration that workflows and integrations reference by name."
---

**Environment variables** are workspace-scoped values your workflows and integrations reference by name at runtime: API endpoints, region codes, feature flags, default tokens, anything that should vary between environments without editing the workflow itself. Set them once on this page and Refold substitutes the current workspace's value wherever a workflow references it.

Open this page from **Settings > Organization > Environment Variables**.
...
Workspace variables are set at the organization level and are available across all workflows and integrations.
