...
<AccordionGroup>
  <Accordion title="A Linked Account for the integration" defaultOpen="true">
    Tests run against a real [Linked Account](/v3/developer/configure/linked-accounts) so any API calls inside the workflow hit the actual third-party app. Pick one from the Testing panel under the Prerequisites section. {/* NEEDS INPUT: confirm panel name in v3 UI is still "Testing" and section heading is still "Prerequisites" */}
...
<Steps>
  ...
  <Step title="Verify in production">
    Trigger one real event and check the run in your [workflow run history](/v3/developer/configure/workflows/overview). {/* NEEDS INPUT: confirm the run-history page path */}
  </Step>
</Steps>
...
- **Use Log nodes as breakpoints.** Drop [Log nodes](/v3/developer/configure/workflows/nodes/log-node) between steps to checkpoint intermediate values during a full-workflow run.
- **Wrap risky calls in Try/Catch.** Test the catch branch by feeding a payload guaranteed to fail (bad ID, missing field). See [Try/Catch nodes](/v3/developer/configure/workflows/nodes/try-catch-node).
...
<CardGroup cols={2}>
  <Card title="Workflow overview" href="/v3/developer/configure/workflows/overview">
    The fundamentals of workflows, triggers, and nodes.
  </Card>
  <Card title="Templating and variables" href="/v3/developer/configure/workflows/overview">
    {/* NEEDS INPUT: replace with the v3 templating page path once it exists */}
    Reference upstream node outputs in your inputs.
  </Card>
  <Card title="Linked Accounts" href="/v3/developer/configure/linked-accounts">
    {/* NEEDS INPUT: confirm Linked Accounts page path in v3 */}
    Authenticate the third-party account your tests run against.
  </Card>
  <Card title="Log node" href="/v3/developer/configure/workflows/nodes/log-node">
