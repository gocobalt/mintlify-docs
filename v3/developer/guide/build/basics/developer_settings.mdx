...
Developer settings page provides you basic organisation specific details such as `API Key` which is required for Refold's api authentication, and `Default app id`.
This section of Refold's platform is dedicated to setup the default application settings which would be useful in other Refold's core functionalities, such as triggering an event, or
...
Use this [API reference](https://docs.gocobalt.io/api-reference/linked-accounts/upsert-linked-account) for a detailed understanding.

However if your api authentication is a bit more complex, such as getting a token using other credentials and using it for the authorization, Refold grants this provision as well. To do so first save the required
...
linked account is being created. Check out this [API reference](https://docs.gocobalt.io/api-reference/linked-accounts/upsert-linked-account) for details.
