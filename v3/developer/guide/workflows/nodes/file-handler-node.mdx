...
The File Handler Node provides comprehensive file management capabilities for workflows, enabling you to transfer files between different systems, manage files in Refold buckets, and interact with SFTP servers. It's essential for workflows that need to process, move, or manage files as part of automated business processes.

**Common use cases:**
- Delete processed Salesforce documents from Refold bucket after attachment
- Transfer Salesforce reports to downstream SFTP-based analytics systems
- Upload Salesforce-generated documents to Refold bucket for processing
...
<Expandable title="Available Actions">
  <ResponseField name="Move file to Refold bucket" type="action">
    Moves a file to Refold bucket storage (deprecated functionality). Legacy action marked for removal in future versions. Use alternative transfer actions for new workflows.
  </ResponseField>
  <ResponseField name="Delete file from Refold bucket" type="action">
    Removes a file from Refold bucket storage using file key or ID. Permanently deletes file from Refold storage. Requires either file key (path) or file ID. Returns success/failure status.
  </ResponseField>
  ...
  <ResponseField name="Transfer file from source to cobalt bucket" type="action">
    Uploads files from external sources to Refold bucket storage. Supports SFTP and URL as source protocols. Files stored in Refold bucket for further processing. Returns publicly accessible URL of uploaded file.
  </ResponseField>
...
<ParamField path="File Key" type="string" required="yes">
  Path/name of the file to delete from Refold bucket. Example: `"documents/invoice_12345.pdf"` or `{{uploaded_file.path}}`
</ParamField>
...
### Transfer to Refold Bucket Output
```json
{
...
This example deletes a processed invoice from Refold bucket after attaching it to a Salesforce record:
...
- **Refold Bucket**: Files stored with automatic naming if not specified
