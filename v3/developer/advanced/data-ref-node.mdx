...
Each [Linked Account](/v3/developer/concepts/linked-accounts) keeps its own Data Refs, even when names match across accounts, so one customer's mappings never leak into another's.
...
<Note>
Refold was previously branded as Cobalt. Some dashboard URLs and image assets still reference `gocobalt.io` while the migration completes.
</Note>
...
<Steps>
  <Step title="Open the dashboard">
    Go to **Settings** > [Data Refs](https://app.gocobalt.io/datarefs).
  </Step>
...
You read from a Data Ref inside a [workflow](/v3/developer/concepts/workflows) by adding the Data Ref node and choosing one of the lookup actions. Reads work in both directions, so you can start from either side of the mapping.
...
<CardGroup cols={2}>
  <Card title="Workflows overview" icon="diagram-project" href="/v3/developer/concepts/workflows">
    Learn how workflow nodes pass data between steps and how Data Ref outputs are consumed downstream.
  </Card>
  <Card title="Linked Accounts" icon="user-plus" href="/v3/developer/concepts/linked-accounts">
    Understand the per-customer scope that Data Refs live inside.
  </Card>
  <Card title="Manage Data Refs" icon="gear" href="https://app.gocobalt.io/datarefs">
    Open the dashboard to inspect, filter, and edit your Data Refs.
  </Card>
  <Card title="Variables and references" icon="brackets-curly" href="/v3/developer/advanced/variables">
