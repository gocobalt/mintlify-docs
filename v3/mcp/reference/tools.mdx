...
The tools an agent sees depend on which toggles are enabled on your [server configuration](/v3/mcp/build/server-configuration). For guidance on choosing between direct mode, agent mode, and skill tools, see [Choose your pattern](/v3/mcp/get-started/choose-your-pattern).
...
| `input_payload` | `object` \| `string` | Yes | Input fields for the action, as a JSON object or a JSON string (e.g., `{"email": "user@example.com"}`) |
...
Returns a compact index of the skills available on this server. The agent browses the index, then loads the full content of a specific entry with `LOAD_SKILL`.
