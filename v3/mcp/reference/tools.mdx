...
The shape of `data` is the third-party app's response passed through, so it varies by action. On failure, the agent receives a generic error message. The full upstream detail is recorded in the audit log, not returned to the agent.
