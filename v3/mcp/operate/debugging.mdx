...
[MCP Inspector](https://github.com/modelcontextprotocol/inspector) is the Model Context Protocol project's open-source debugger for any MCP server.
...
2. Copy the returned `slug`, `type`, `identifier`, and `json_schema`
...
<Note>
Downstream API errors from third-party apps are not surfaced verbatim to the agent. The agent receives a clean, user-facing message, and the detail panel shows that same message. The full upstream error from the third-party API is not exposed.
</Note>
