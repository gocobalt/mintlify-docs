...
<Steps>
  ...
  <Step title="Paste your Server URL">
    Enter your Refold Server URL. Refold accepts the token two ways, so use whichever the Inspector supports.
...
### The agent sees no tools, or not the tools you expect
...
- **Enabled actions and skills.** Only the apps and actions you added to the server appear as tools, so add any missing action on the server's **Applications** tab. The skill tools `LOAD_SKILL` and `GET_KNOWLEDGE_INDEX` appear only when **Retrieve Skill** is on, in either mode.
...
<Steps>
  ...
  <Step title="Check the Status field">
    ...
    |---|---|
    | `success` | Refold completed the call. The issue is on the agent's interpretation side, so compare the response payload to what the agent told the user. |
    | `validation_error` | The agent sent input that didn't match the tool's schema, referenced an app or action that isn't on the server, or hit a known business-rule rejection. The message the agent received explains what failed. |
    | `execution_error` | Refold couldn't reach the downstream service. The agent receives a generic message, and the call's detail panel shows the full error. |
    | `pending` | The call started but did not record completion. |
  </Step>
</Steps>

<Note>
Downstream API errors from third-party apps are not surfaced verbatim to the agent. The agent receives a generic message, and the full upstream error is recorded in the call's detail panel.
</Note>
