...
- **[MCP Logs](/v3/mcp/operate/logs)** is the audit panel in the Refold dashboard. It shows every tool call your agent made, the exact input it sent, and what came back.
...
See [Server configuration](/v3/mcp/build/server-config) for where these toggles live and [How it works](/v3/mcp/overview) for how each mode changes the tool list.
...
Once your agent is running, every tool call is captured in **Maintain > MCP Logs**. See [MCP Logs](/v3/mcp/operate/logs) for the full field reference, filters, and compliance details.
