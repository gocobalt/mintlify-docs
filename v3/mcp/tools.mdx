...
The tools an agent sees depend on which toggles are enabled on your [server configuration](/v3/mcp/build/server-config). For guidance on choosing between direct mode, agent mode, and skill tools, see [Choose an access pattern](/v3/mcp/build/server-config#choose-an-access-pattern).
