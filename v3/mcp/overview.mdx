...
Every tool call is authenticated and logged in [MCP Logs](/v3/mcp/operate/mcp-logs).
...
**Skills** add pre-built procedures the agent can load in either mode. **Workflows** run server-side as one call. See [Choose an access pattern](/v3/mcp/build/server-configuration#choose-an-access-pattern) for the decision framework.
