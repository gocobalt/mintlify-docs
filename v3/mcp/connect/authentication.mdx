...
MCP authentication builds on Refold [linked accounts](/v3/platform/concepts/linked-account). Every MCP server connection carries a token embedded in the Server URL that identifies one linked account, and Refold validates it on every request. The same model that connects your customers' third-party accounts also authenticates their agents — see [Linked accounts](/v3/platform/concepts/linked-account) for how those accounts and their credentials are established.

This page covers what is MCP-specific: the Server-URL token, how access is scoped to the apps and actions you expose, and how to revoke a token. For the connection flows that produce a linked account in the first place — hosted, build-your-own, re-authentication, and more — see the [authentication hub](/v3/platform/authentication/overview).
...
The agent never handles third-party credentials. Refold resolves the linked account's stored OAuth tokens and API keys internally when a tool executes, so credentials stay encrypted at rest and are never returned in tool inputs or outputs. For where those credentials come from, see [Connectors](/v3/platform/concepts/connector/supported-apps-actions); for how end users authorize them, see the [hosted auth flow](/v3/platform/authentication/connection-flows/hosted-flow).
...
- [Connectors](/v3/platform/concepts/connector/supported-apps-actions) — where the underlying OAuth and API-key credentials are configured
