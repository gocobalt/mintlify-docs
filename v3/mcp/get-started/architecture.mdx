...
This page covers the MCP-specific model: what an MCP server is built from, how your actions, workflows, and skills become tools, and the path a request takes from agent to app. For the decision of *which* tool pattern to expose, see [Choose your pattern](/v3/mcp/get-started/choose-your-pattern).

## Core MCP objects

A Refold MCP server is built from objects you already configure in Refold. Three are MCP-specific; the rest are [platform objects](/v3/platform/concepts/linked-account) you connect to from MCP.
...
The objects an MCP server draws from live in Platform. Each links to its full reference:
...
- A [linked account](/v3/platform/concepts/linked-account) is one end user's connection to those apps. Its credentials are what every tool call runs under.
- A [platform workflow](/v3/platform/concepts/workflows/overview) is a multi-step process you build once and can expose as a single MCP tool.
...
Refold validates the token and server ID on each request. A valid token scopes the request to exactly one linked account, so every tool call in that request runs under one customer's connection and one tenant's data, in the test or production environment the token belongs to. The mode and skill settings come from the MCP server's dashboard configuration, resolved server-side per request. This scope is fixed for the life of the request, and a tool call cannot widen it.
...
When the Retrieve Skill toggle is on, the skill tools `GET_KNOWLEDGE_INDEX` and `LOAD_SKILL` are added in either mode.
...
The MCP server keeps no session state between requests. Each request carries its own token, and Refold resolves the linked account, configuration, and credentials from scratch every time. Because no request depends on a previous one, Refold can serve many customers at once and update the service without interrupting connected agents.
