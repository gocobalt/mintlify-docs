...
Mode is exclusive, but layers stack. A server in either mode can also expose skills, attached workflows, or both. Most production servers settle on one mode and add a workflow or two for the high-stakes processes.
...
| **You define** | The exact set of exposed actions | The action set the agent resolves over | The procedures (skills) the agent can follow | The full process: steps, sequencing, retries, rollback |
...
A [workflow](/v3/platform/concepts/workflows/overview) you build in Refold attaches to the server and appears as a single tool, in either mode. The agent calls it once with input, the workflow handles sequencing, retries, and rollback server-side, then returns one result. The agent never touches the intermediate steps.
