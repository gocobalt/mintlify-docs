...
A [workflow](/v3/concepts/workflows/overview) you build in Refold attaches to the server and appears as a single tool, in either mode. The agent calls it once with input, the workflow handles sequencing and error handling server-side, then returns one result. The agent never touches the intermediate steps.
