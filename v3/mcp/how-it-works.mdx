...
The sections below explain what an MCP server is built from, how your actions, workflows, and skills become tools, and the path a request takes from agent to app. For the decision of *which* tool pattern to expose, see [Choose your pattern](/v3/mcp/choose-your-pattern).
...
How actions appear depends on the server's mode. In direct mode, every selected action and workflow is exposed as its own tool. In agent mode, actions and workflows are reached through `RESOLVE_ACTIONS` and `EXECUTE_ACTION` instead. See [Choose your pattern](/v3/mcp/choose-your-pattern) for the trade-offs.
...
- [Choose your pattern](/v3/mcp/choose-your-pattern): direct, agent, and workflow tool patterns
- [Server configuration](/v3/mcp/build/server-configuration): set up an MCP server in the dashboard
- [Tools](/v3/mcp/tools): the full tool reference and schemas
- [Authentication](/v3/authentication/mcp): the Server-URL token and access toggles
