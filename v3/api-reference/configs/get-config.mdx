...
Retrieve an installed config for an app and a specific `config_id`. Returns the fields and workflows for the installation. (To use the default config for a linked account, omit the ID — see [Get Config](/v3/api-reference/configs/get-config-default).)
