...
<Card title="Download the OpenAPI spec" icon="download" href="https://raw.githubusercontent.com/gocobalt/mintlify-docs/main/v3/api-reference/openapi.yaml" horizontal>
  Grab the full OpenAPI 3 specification to generate SDK clients, import into Postman or Insomnia, or explore the API offline.
</Card>

<Info>
All API requests use the base URL `https://app.refold.ai`. We've updated our domain to `app.refold.ai` from `app.gocobalt.io` — existing `app.gocobalt.io` requests will continue to work until it is decommissioned at a later date.
</Info>
