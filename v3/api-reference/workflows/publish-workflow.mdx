...
<Note>
  This requires a session token, so it operates on the linked account's **private** workflow.
</Note>
...
<Note>
  **Authentication:** this endpoint requires a **session token** (`Authorization: Bearer ...`). Issue one with [Generate Session Token](/v3/api-reference/session-tokens/generate-session-token).
</Note>
