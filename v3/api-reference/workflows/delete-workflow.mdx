...
<Note>
  This requires a session token, so it deletes the linked account's **private** workflow.
</Note>

<Note>
  **Authentication:** this endpoint requires a **session token** (`Authorization: Bearer ...`). A version-snapshot workflow cannot be deleted directly — delete the parent workflow instead.
</Note>
