...
<Info>
  **Base URL: `https://app.refold.ai`.** As part of the Refold rebrand, the API's previous domain `https://api.gocobalt.io` has moved to `https://app.refold.ai`. Existing `api.gocobalt.io` requests continue to work for now, but that domain may be sunset at a later point — point new integrations at `https://app.refold.ai`.
</Info>
