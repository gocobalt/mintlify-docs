...
<ParamField header="x-api-key" type="string" required>
  Organization API key, sent in the `x-api-key` header. Production keys start with `pk`, test keys with `tk`.
</ParamField>
            ...
            Date for the schedule.
                <Expandable title="properties">
                  <ResponseField name="hour" type="string" required>
                        Hour of the timestamp Eg: 21
                    </ResponseField>
                  <ResponseField name="minute" type="string" required>
                        Year of the date. Eg: 45
                    </ResponseField>
...
```bash cURL
curl --request POST \
  --url https://app.refold.ai/api/v1/workflow/public/scheduled \
  --header 'Content-Type: application/json' \
  --header 'linked_account_id: <linked_account_id>' \
  --header 'slug: <slug>' \
  --header 'workflow_id_alias: <workflow_id_alias>' \
