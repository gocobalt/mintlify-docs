...
<Note>
**Token in URL.** The token is embedded in the Server URL path rather than an `Authorization` header because not every MCP client supports custom headers today. Treat the Server URL as a secret — store it like an API key, rotate it the same way, and prefer per-end-user URLs over shared ones. Header-based auth is on the roadmap as MCP clients add support.
</Note>
...
Audit records are partitioned by org and queryable from the dashboard. See [MCP Logs](/mcp/logs) for filtering and search, and [Architecture](/mcp/architecture) for how a single tool call flows end to end.
...
The following are scoped for upcoming releases:

- **Header-based auth** — bearer token in an `Authorization` header instead of the URL path, as MCP clients add support
- **Per-user identities** — multiple end users sharing a single linked account, each with their own audited identity
- **Tool-level RBAC** — fine-grained permissions on which tools each identity can call
