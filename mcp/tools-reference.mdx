...
Each tool has a fixed schema. The `application_slug`, `action_id`, and `type` fields are hardcoded as constants so the agent cannot change them. The only thing the agent fills in is the `input_payload`.
...
| `related_entity_name` | `string` | `null` | Optional related entity to find connections between |
| `apps` | `list[string]` | `null` | Filter to specific app slugs |
| `min_confidence` | `float` | `0.5` | Minimum confidence threshold for results |
| `limit` | `int` | `25` | Maximum number of results |
| `depth` | `int` | `1` | Graph traversal depth |
| `max_response_chars` | `int` | `12000` | Cap on response size |
| `path_mode` | `"paths"`, `"ego_union"`, or `"both"` | `"both"` | Graph query strategy |
