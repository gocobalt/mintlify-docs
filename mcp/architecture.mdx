...
For the security model behind this — token validation, tenant isolation, audit guarantees — see [Security & Governance](/mcp/security-governance).
...
The token is in the URL path rather than an `Authorization` header because not all MCP clients support custom headers today. Treat the Server URL as a credential. See [Security & Governance](/mcp/security-governance#authentication) for the full auth model and roadmap.
