...
The tool set is resolved on every `tools/list` and `tools/call` request from the validated identity. Tools are not cached across requests, which means two concurrent sessions never share tool objects.
...
The MCP server holds no session state between requests. Identity, configuration, and credential data are resolved per-request. This is what makes horizontal scaling and tenant isolation work, and why a deployment can be replaced without breaking active sessions.
