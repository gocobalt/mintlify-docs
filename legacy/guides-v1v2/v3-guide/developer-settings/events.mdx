...
Events are the trigger entities that are used to define a start point of a workflow and can be triggered using the [REST APIs](/legacy/api-reference/events/trigger-event-by-slug) or the Event URL assigned for the Organization.
Think of these custom events as signals sent from your app using the Refold SDK or Event API to kick off different processes. Refold events are `common models`. This means 
...
<img height="200" src="/images/Developer/event-trigger.png" alt="Selecting an event trigger"/>
...
```curl
curl --request POST \
  --url https://app.refold.ai/api/v1/webhook/64c357c739ec788238ab5f9b \
...
instead of for all application workflows with which the event is associated, kindly refer the [REST APIs](/legacy/api-reference/events/trigger-event-by-slug) support for events.

To trigger an event from Refold platform so that you can simulate event execution, test workflows and validate the behavior of associated actions, you can use the `Try API` feature of Refold. Refer [here](/legacy/guides-v1v2/platform/tryapi) to learn how you can use Try API.
