...
Events are the trigger entities that are used to define a start point of a workflow and can be triggered using the [REST APIs](https://docs.gocobalt.io/api-reference/events/trigger-event-by-slug) or the Event URL assigned for the Organization.
...
instead of for all application workflows with which the event is associated, kindly refer the [REST APIs](https://docs.gocobalt.io/api-reference/events/trigger-event-by-slug) support for events.
