...
Use the Upsert Linked Account API call below or the **.upsertLinkedAccount()** method of the [NodeJS SDK](/legacy/api-reference/sdks) to create or update a linked account.
<CodeGroup>
```bash cURL
curl --request PUT \
...
You can check the Linked Accounts present in your account by using the List Linked Accounts API Or through the **Refold Dashboard** by navigating to `Linked Accounts` in the side menu.
