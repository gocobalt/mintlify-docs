...
[Linked Accounts](/legacy/build/basics/linked_account) refers to the end-users or customers of our users who utilize the integrations built using the Refold platform.
...
2. **Event based triggers** which triggers when an event is called on Refold. You can check which Event is added to the workflow by checking the **Start Node** of the workflow.
    ...
    - To start the workflow execution, go to [**Try API**](/legacy/ship/tryapi) section in Refold, provide the required details and click on `Fire Event`.
