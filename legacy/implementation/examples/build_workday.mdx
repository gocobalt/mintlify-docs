...
In this guide, we will walk through the essential steps required to build a Workday integration using the Refold platform. This process involves three major steps i.e. User Authentication with Workday, Creating Workflows and Using workflows for users.
...
After setting up your account, the next step is to enable the Workday app in your Refold account.

<Note>Apps cannot be enabled using the API. You will need to enable the app through the UI.</Note>

To enable the Workday app:

Navigate to `Apps` in Refold and search for **Workday** and enable the app by clicking on the `Go Live` button in the top right corner.
...
<Tip>
Follow the steps given [here](/applications/providers/workday) to get your API credentials required for authentication.
</Tip>
...
A Linked Account in Refold represents the end-users or customers who will use the integration through the platform. Each Linked Account requires a unique linked_account_id, which typically corresponds to an ID from your internal data model (e.g., a user or account ID).

<Tip>
Learn more about Linked Accounts [here](/legacy/build/basics/linked_account).
</Tip>
...
```JavaScript cURL
    curl --request POST \
  --url https://app.refold.ai/api/v2/public/session-token \
  --header 'Content-Type: application/json' \
  --header 'x-api-key: <api-key>' \
  --data '{
  "linked_account_id": "<string>"
...
```JavaScript cURL
curl --request POST \
  --url https://app.refold.ai/api/v2/public/connect-url \
  --header 'Content-Type: application/json' \
  --header 'x-api-key: <api-key>' \
  --data '{
  "color": "<string>",
...
<Steps>
  ...
  <Tip>
  Learn more about the triggers and its types [here](/legacy/build/workflow/triggering_workflow).
  </Tip>
...
<Tip>Learn more about API Proxies [here](/legacy/build/basics/proxies).</Tip>
</Step>
...
<Steps>
  ...
  ## Create Config
You can create config for the Linked Account through the Refold Connect SDKs or using the APIs.

You can use the **.config()** method or the [Create Config API](/legacy/api-reference/configs/create-config) which returns the specified config or creates one if it doesn't exist for the Linked Account.
...
```
  ...
  <Step title="Enable Workflow">
    Now you need to enable the workflow for your Linked account. You can either ask the user to enable them or you can do so for them by using the [Update Config API](/legacy/api-reference/configs/update-config) or **.updateconfig()** method.
...
In response, you get the updated config information for the application.
  ...
  <Step title="Third Step">
    Once the required workflows are enabled, you need to fire event to start execution. You can use the [Trigger Event for app](/legacy/api-reference/events/trigger-event) API with the following request:
    ```JavaScript cURL
  curl --request POST \
    --url https://app.refold.ai/api/v2/public/event/:slug \
    --header 'Content-Type: application/json' \
    --header 'linked_account_id: <linked_account_id>' \
    --header 'x-api-key: <api-key>' \
    --data '{
  ...
  Once the workflow execution has been fired, you can check the Logs of the workflow [here](https://app.refold.ai/logs/executions).

  <Tip>
Learn more about logs and observability [here](/legacy/maintain/logs).
  </Tip>
  </Step>
