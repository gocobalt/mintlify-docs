...
<Note>
Know more about Linked Accounts and how you can create one in the guide [here](/legacy/concepts/linked_account).
</Note>
...
<Tip>
Check out the [Refold's API Reference](/legacy/api-reference/connect/create-connect-url) for more details. 
</Tip>
  </Step>
  ...
  <img height="200" src="/images/Guides/hosted-portal.png" alt="Hosted Portal in Refold"/>
  </Step>

</Steps>


You can modify the UI of Hosted Portal using the [hosted_properties](/legacy/api-reference/hosted-properties/get-hosted-properties).
...
| bgColor | Background Colour of your end customer |
  ...
  It's a simple process involving steps like `Generate Session Token` > `Install and Initialise Frontend SDK` and `Render the Component`.
    ...
    <Info>
    Learn more about Refold's React SDK and how to initialize it [here](/legacy/api-reference/sdks).
    </Info>
...
Retrieve a list of all integrations available to connect using the [List Applications API](/legacy/api-reference/applications/list-applications) or [.getApplications()](/legacy/api-reference/sdks) method and display them in your UI.
<CodeGroup>
...
<Tip>
Learn more about Refold’s JS SDK and how to initialize it [here](/legacy/api-reference/sdks).
</Tip>

  </Accordion>

  <Accordion title="Manage Connections">
Refold provides the flexibility for you to manage your customers' application connections. The [`.connect()`](/legacy/api-reference/sdks) method
...
The [`.disconnect()`](/legacy/api-reference/sdks) method similarly, lets the user to disconnect from the application platform, revoking
...
Once the user has successfully authenticated in their platform, he is redirected back to your app integration page. Use the [`.config()`](/legacy/api-reference/sdks) method to find an 
...
enter data for the settings and map fields. These updates to the config can be made using the [`.updateConfig()`](/legacy/api-reference/sdks) method.
