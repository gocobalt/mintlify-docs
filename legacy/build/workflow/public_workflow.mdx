...
description: Private Workflows in Refold allow linked accounts to create and execute their own workflows.
---
...
```bash
<iframe
    src={ `https://app.refold.ai/workflows/${ workflowId }?token=${ cobaltSessionToken }` }
    height="100%"
    width="100%"
    style={{
        border: "none",
...
/>
```
You need to provide the [Workflow Id](/legacy/api-reference/workflows/list-workflows) and [Session Token](/legacy/api-reference/session-tokens/generate-session-token).
...
curl --request POST \
  --url https://app.refold.ai/api/v2/public/workflow \
...
```
<Tip>
Learn more about the Create Private Workflow API [here](/legacy/api-reference/workflows/create-workflow).
</Tip>

To create a draft or editable version of the workflow, use the following cURL:
```bash Create Draft version
curl --request GET \
  --url https://app.refold.ai/api/v2/public/workflow/:workflow_id/edit \
  --header 'Authorization: Bearer {Session_token}' \
```
<Tip>
Learn more about the Create Draft of Workflow API [here](/legacy/api-reference/workflows/get-draft).
</Tip>
