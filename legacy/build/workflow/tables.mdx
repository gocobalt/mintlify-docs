---
title: Tables by Refold
description: Tables by Refold allows users to manage and use dynamic data generated within workflows and are structured with rows and columns.
---


## Tables Types
The type of a Table can be chosen by the user when the **Create Table** action is selected in the `Tables by Refold` node.
<Info>
...
All persistent tables created in the Refold platform can be accessed by navigating to `Settings` > [Tables](https://app.refold.ai/tables). This section provides details about each table, such as its ID and Name.
