...
| **Workflow API** | Programmatic invocation without a pre-defined event. Useful for batch orchestration and workflows called from other workflows via the [Sub Flows node](/legacy/build/workflow/sub_flows). |
...
If the same logic is needed in multiple workflows, create a reusable [Internal Function](/legacy/build/workflow/internal_functions) rather than duplicating Custom Code nodes. Internal Functions are defined globally in `Advanced → Functions` and can be called from the Functions node in any workflow.
...
The [Tables node](/legacy/build/workflow/tables) provides structured, row-and-column storage within workflows. Two types are available.
...
The [HTTP node](/legacy/build/workflow/httpnode) is the right tool for public APIs and simple calls where auth is handled externally. It is not the right tool for APIs requiring OAuth flows, managed API keys, or automatic token refresh.

For these APIs, use a [Custom App](/legacy/custom_app/custom_app). Custom Apps provide:
...
**[Encrypted Output Fields](/legacy/build/workflow/encrypted-output-fields)** are Refold's implementation of output-level PII masking. They let you mark individual output fields on any node so their values are stored securely and shown as `[REDACTED]` in the Run tab and execution logs — without affecting how values flow to downstream nodes at runtime.
...
See the [Encrypted Output Fields](/legacy/build/workflow/encrypted-output-fields) page for full configuration details and field path syntax.
...
Once individual nodes are validated, use Refold's built-in [Workflow Testing](/legacy/build/workflow/workflow_testing) to run the full workflow against a sample payload. This surfaces integration-level issues — missing references between nodes, auth failures, and unexpected response shapes from external systems.
...
For custom JavaScript logic used across multiple workflows, create a global [Internal Function](/legacy/build/workflow/internal_functions) (`Advanced → Functions`) rather than duplicating a Custom Code node. Changes to the function automatically apply everywhere it is used.
...
Use the [Sub Flows node](/legacy/build/workflow/sub_flows) to call one workflow from within another. This is effective for:
...
**All Automation Agent nodes should be migrated to [Custom Code nodes](/legacy/build/workflow/custom-code).**
...
*For node-specific documentation, see the [Workflows section](/legacy/build/workflow/overview). For authentication and Custom App setup, see [Custom Apps](/legacy/custom_app/custom_app). For platform updates, see the [Changelog](/legacy/changelog/overview).*
