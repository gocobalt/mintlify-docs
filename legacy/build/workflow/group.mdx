...
title: Loop Node
---
The Loop Node in Refold allows users to iterate over entities or execute a series of actions for a fixed number of iterations.
...
- Use the data received from all iterations of the Loop node in other nodes of workflow. Refer [here](/legacy/build/workflow/group#group-node-output);
- Execute nodes for fixed number of times using [Fixed Iteration](/legacy/build/workflow/group#actions);
- Execute as many times as there are items in an array using [Array Iteration](/legacy/build/workflow/group#actions); &
- Execute nodes repetitively based on case and break conditions using [Do While](/legacy/build/workflow/group#actions);
...
<img height="200" src="/images/Guides/Workflow/group-node.png" alt="Loop Node in Refold"/>
...
Let's consider a simple example to understand Do While by creating a [workflow](/legacy/build/workflow/overview). In this, we will execute the loop with the condition that it will be executed until the iteration count is 5 and will break out of loop if iteration count is greater than or equal to 3.
<Steps>
...
Add a [Custom Code](/legacy/build/workflow/custom-code) and a [Response node](/legacy/build/workflow/response) inside the Loop Node.
</Step>
...
<img height="200" src="/images/Guides/Workflow/iterating.png" alt="Iterating Array Action in Refold"/>
...
<img height="200" src="/images/Guides/Workflow/output.png" alt="Output of Loop Node in Refold"/>
...
```
<Tip>
To know more about Templating in Refold, refer to our doc [here](/legacy/build/workflow/templating).
</Tip>