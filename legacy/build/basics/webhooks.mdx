---
title: Webhooks
description: Learn how to setup Webhooks in Refold
---
...
You can manage the webhook subscription using our webhooks API. Visit the [API reference](/legacy/api-reference/webhooks/create-webhook) 
...
For example, to fire the [Workflow API](/legacy/build/workflow/workflow_api) of Refold to execute a workflow when some event occurs in Refold. You will simply add the headers and Body too if required.
...
If a workflow with [Response node](/legacy/build/workflow/response) in it has executed successfully and you want to use its response data during webhook firing, you can template that in your headers or body.
...
// your Refold API key based on environment
const WEBHOOK_SECRET = process.env.WEBHOOK_SECRET || '<Refold-API-key>';
...
/**
 * Verifies the HMAC signature from a webhook received from Refold
...
# your Refold API key based on environment
WEBHOOK_SECRET = os.getenv("WEBHOOK_SECRET", "<Refold-API-key>")
