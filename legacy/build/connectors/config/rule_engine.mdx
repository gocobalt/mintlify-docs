...
Consider an example that you can set up a rule to provision a user in an application only if their type is **Agent** and they have a specific role.
    <img height="200" src="/images/Config/rule_engine_hosted.png" alt="Rule Engine by Refold"/>
...
<Steps>
  ...
  <Step title="Create Dataslot">
    ...
    <img height="200" src="/images/Config/rule_engine.png" alt="Rule Engine field type by Refold"/>
  </Step>
</Steps>
...
async function main() {
    ...
    if (lhs === "Region") {
        ...
        // log the rhs options. you'll see the logs when you call the `getRuleFieldOptions` method in Refold SDK
