...
<Steps>
  ...
  <Step title="Convert Vendor data into a Key Value pair">
    ...
    We can do this either by [Transform Node](/legacy/build/workflow/transform) or [Custom Code](/legacy/build/workflow/custom-code) node.
  </Step>
  ...
  <Step title="Send transformed data in Response node">
    As a final step of every DataSource workflow, you need to send that data in a [Response node](/legacy/build/workflow/response) so that it's visible to your users as options in Config.
    <Info>
