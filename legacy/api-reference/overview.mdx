...
<Info>
All API requests use the base URL `https://app.refold.ai`. We've updated our domain to `app.refold.ai` from `api.gocobalt.io` — existing `api.gocobalt.io` requests will continue to work until it is decommissioned at a later date.
</Info>
...
<CardGroup cols={2}>
  ...
  <Card title="Credentials" icon="key" href="/v3/api-reference/credentials/get-credentials">
    Read tokens, save key-based credentials, and migrate existing auth onto an integration.
  </Card>
