...
Return a flat description of the auth fields required for each of an app's supported auth types (`oauth2`, `keybased`, …). Use this to build a credential-entry form dynamically, or to know exactly which fields to send to [Save Key Credentials](/legacy/api-reference/credentials/save-key-credentials) or [Migrate Token](/legacy/api-reference/credentials/migrate-token).
