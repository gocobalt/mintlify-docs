...
Delete the config installation for an app on a linked account, without specifying a `config_id` — it defaults to the linked account ID. To target a specific config ID, use [Delete Config by ID](/legacy/api-reference/configs/delete-config).
