...
description: To diplay the apps to your end-users in the frontend, you can develop your own custom UI or redirect your users to Refold Connect, which is securely hosted by Refold.
---
...
To enable the apps that you want to use in Refold, go to the Apps page in the Refold dashboard. You will see a list of the available apps. Choose one or more apps to enable. The apps that you enable will become available to your users in the Connect flow.
<img height="200" src="/images/Concepts/app-list.png" alt="Apps in Refold"/>

**Key-based** apps do not require any additional configuration.

**OAuth** apps by default use Refold credentials. This makes it easy to get started and connect integrations right away. However, once you’re ready to ship your integrations with Refold, you should use your own OAuth app credentials (client ID and client secret). This will make sure that users see your brand instead of Refold when they connect integrations.

<Note>
You can find the App Setup instructions for some of the popular integrations that we support [here](/legacy/build/connectors/overview).
</Note>

## Building Custom UI
If you want to build a custom UI for your integrations, you can do so by using the Refold APIs or the Refold Connect SDK available in JavaScript and React. 
<AccordionGroup>
  <Accordion title="Refold API for UI">
If you want to create your own frontend for the integration and want to fetch information about the various integrations that will be available to the end-users, you can do so by using the [List Applications API](/legacy/api-reference/applications/list-applications).
...
You can also fetch information about a single application by providing the app slug in the [Get Application API](/legacy/api-reference/applications/get-application).

  </Accordion>

  <Accordion title="Refold SDK for UI">
If you prefer to develop using the SDKs provided by Refold, then the **.getApp()** method will get you the list of applications enabled in Refold. It returns the application details for the specified application, provided the application is enabled in Refold. If no application is specified, it returns all the enabled applications.
...
2. **Response** : The instance method of Refold Returns: `Promise.<Application>` - The application details. 
``` JavaScript Response
{
	"name": "Slack",
...
```
<Note>
Learn more about the Embedded and Seamless flow of Refold Connect [here](/legacy/concepts/auth-flows/auth-flows).
</Note>
  </Accordion>
</AccordionGroup>
...
<Check>
You have now successfully built a Custom UI for your end users which displays all the integrations enabled for them. Next step is to [Manage Connections](/legacy/implementation/integrate/manage-connection) for your customers.
</Check>