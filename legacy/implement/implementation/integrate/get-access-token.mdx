...
1. **Request** : In your server side code, make a POST Request to the `/session-token` endpoint or call the [**.getTokenForLinkedAccount()**](/legacy/api-reference/sdks) method of the **NodeJS SDK**. The request should include the `linked-account_id` of the user for which the Session token is to be generated.
<Warning>
...
<Check>
After completing this step, you now have the Session Token for your Linked Accounts. But before you make the API Calls, first let's [Display the apps/integrations on Client-side](/legacy/implementation/integrate/display-apps).
</Check>